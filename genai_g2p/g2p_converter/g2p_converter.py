...
class G2PConverter:
    def __init__(self, provider, model_name, mode = 'predict', g = 'English', p = 'cmudict', system_prompt = None, max_split_length = 1000, **kwargs):
    ...
    def build_default_system_prompt(self, mode = 'predict', g = 'English', p = 'cmudict') -> str:
        if mode == 'predict':
            return f"You are a linguistic expert specialized in phonetic transcription from {g} to {p}.\nProvide accurate phonetic transcriptions for each line without extra explanation.\nIf explanation is necessary, provide them in brackets."
        elif mode == 'correction':
            return f"You are a linguistic expert specialized in phonetic transcription from {g} to {p}.\nProvide correction of the existing phonetic transcriptions for column 1 in each line without extra explanation.\nColumn 1 is the text, column 2+ is the existing phonetic transcription.\nThe existing transcription may not be accurate, so think carefully and correct them if necessary.\nIf explanation is needed, provide them in brackets."
